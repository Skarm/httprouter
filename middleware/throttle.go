...
// at a time.
func Throttle(limit int) func(httprouter.Handler) httprouter.Handler {
...
// pending requests.
func ThrottleBacklog(limit int, backlogLimit int, backlogTimeout time.Duration) func(httprouter.Handler) httprouter.Handler {
	if limit < 1 {
	...
	fn := func(h httprouter.Handler) httprouter.Handler {
		t.h = h
		return &t
	}
...
func (t *throttler) ServeHTTP(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	ctx := r.Context()
	select {
	case <-ctx.Done():
